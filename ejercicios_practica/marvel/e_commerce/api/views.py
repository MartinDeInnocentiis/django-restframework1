...
def comic_list_filtered_api_view(request):
    filtered_comics = Comic.objects.filter(price__gte=5.00)
    _data = list(filtered_comics.values()) if filtered_comics.exists() else []
    return Response(data=_data, status=status.HTTP_200_OK)



@api_view(http_method_names=['GET'])
def comic_list_advanced_filter_api_view(request):
    avengers_comics = Comic.objects.filter(Q(title__icontains='x-men') | Q(title__icontains='Spider'),
        price__gt=5.99)
    _data = list(avengers_comics.values()) if avengers_comics.exists() else []
    return Response(data=_data, status=status.HTTP_200_OK)
